...
def generate(data_groups, depths, thresholds, preds, mls, treatment_options, array, slurm_file):
        ...
        S+="\n"
        S+="\n"
        command = 'python main_agg.py ' + '-f ' +'${train_data_list[$SLURM_ARRAY_TASK_ID]}' + ' -e ' +'${test_data_list[$SLURM_ARRAY_TASK_ID]}' + " -d " + '${depth_list[$SLURM_ARRAY_TASK_ID]}' + " -t " + str(time_limit) + " -b " + str(100)+ " -p " + '${pred_type_list[$SLURM_ARRAY_TASK_ID]}'+ " -r direct" + " -g " + '${data_group_list[$SLURM_ARRAY_TASK_ID]}' + " -m " + '${ml_list[$SLURM_ARRAY_TASK_ID]}' + " -u " + '${treatment_options_list[$SLURM_ARRAY_TASK_ID]}'
        S+=command
        S+="\n"
...
def main():
    ...
    slurm_file = 'slurm_DM_synthetic.sh'
    depths = [1]
    ...
    slurm_file = 'slurm_DM_warfarin_r.sh'
    depths = [1, 2, 3, 4]
    ...
    data_groups = ['Warfarin_seed1','Warfarin_seed2','Warfarin_seed3','Warfarin_seed4','Warfarin_seed5'] #Warfarin_3000 Athey_v1_500  Athey_v2_4000
    slurm_file = 'slurm_DM_warfarin_0.33.sh'
    depths = [1, 2, 3, 4]
    thresholds = ["0.33"]
    preds = ['tree']
    mls = ['ml']
