...
def generate(data_groups, depths, thresholds, array, slurm_file):
        ...
        S+="\n"
        S+="\n"
        command = 'python main.py ' + '-f ' +'${train_data_list[$SLURM_ARRAY_TASK_ID]}' + ' -e ' +'${test_data_list[$SLURM_ARRAY_TASK_ID]}' + " -d " + '${depth_list[$SLURM_ARRAY_TASK_ID]}' + " -t " + str(time_limit) + " -b " + str(100)+ " -r kallus" + " -n 0" + " -g " + '${data_group_list[$SLURM_ARRAY_TASK_ID]}'
        S+=command
        S+="\n"
...
def main():
    ...
    slurm_file = 'slurm_K-PT_synthetic.sh'
    ...
    slurm_file = 'slurm_K-PT_Warfarinv2.sh'
